...
void kmeans(uint8_t k, cluster* centroides, uint32_t num_pixels, rgb* pixels){    
      ...
      //inicialitzem auxiliars

  uint32_t centroides_arrayR[k], centroides_arrayG[k], centroides_arrayB[k];
  ...
  uint32_t* points=malloc(k *sizeof(uint32_t));
...

      {
                  ...
                  //inicialitzem vectors auxiliars
                  red[j] = 0;
                  green[j] = 0;
                  blue[j]=0;
                  points[j]=0;
...
for(j = 0; j < num_pixels; j++)
  ...
  //Actualitzem les variables originals
